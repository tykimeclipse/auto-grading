...
--     - grade_level이 NOT NULL이면 course_name에서 추론
-- =========================================================
...
declare
  ...
  v_grade_level_nullable boolean;
  v_subject_group_nullable boolean;
...
begin
  ...
  if v_has_grade_level and coalesce(v_grade_level_nullable, true) = false and v_inferred_grade_level is null then
    raise exception 'grade_level 컬럼이 NOT NULL인데 course_name에서 학년을 추론할 수 없습니다. 제목에 중1/중2/중3/고1/고2/고3 중 하나가 포함되어야 합니다.';
  end if;

  if v_has_subject_group and coalesce(v_subject_group_nullable, true) = false and v_inferred_subject_group is null then
...
is '현재 운영 courses 테이블 구조에 맞춘 안전한 등록 RPC. 존재하는 컬럼만 insert 하며, grade_level이 NOT NULL이면 제목에서 학년 코드를 추론한다.';
