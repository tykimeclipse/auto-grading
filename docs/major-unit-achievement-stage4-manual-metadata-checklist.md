...
- 인자 순서는 제목, 총 문항수, 시험 내용 학년, 교육과정, 교육과정 과목,
  단원코드다.
- 네 메타데이터 값은 모두 필수이며 단원코드는 3자리 숫자다.
- 현재 중등 과목 식별자는 `중1과학`, `중2과학`, `중3과학`처럼 공백 없이
  저장하고 화면에서만 `중1 과학`처럼 표시한다. 교육과정 단원 업로드 화면은
  알려진 중등 과학·고1 통합과학 식별자의 공백 변형을 저장 전에 정규화한다.
...
- 교육과정 단원 업로드 화면 하단에서는 활성·비활성 단원을 모두 조회할 수
  있으며 교육과정 → 학년 → 과목 → 대단원 → 중단원 → 소단원을 순서대로
  선택한다. 긴 전체 목록 대신 선택한 단원 경로의 최대 3개 row만 표시한다.
...
2. 교육과정 선택 전에는 시험 내용 학년·교육과정 과목·단원 선택이 비활성
   상태다.
3. 교육과정 → 시험 내용 학년 → 교육과정 과목 순서로 선택하면 활성 단원만
   표시된다.
...
10. 교육과정 단원 업로드 화면 하단에서 등록 조합과 조합별 단원 수를 확인한다.
    기본 활성 필터와 비활성·전체 필터가 동작하고, 교육과정 → 학년 → 과목 →
    대단원 → 중단원 → 소단원 드롭다운이 상위 선택에 맞춰 연동되어야 한다.
    결과 표에는 선택한 대·중·소단원 경로만 최대 3행으로 표시되어야 한다.
